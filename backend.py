...
COMPANY_URLS = [
    "https://www.sozhaa.events/",
    "https://www.sozhaa.events/about-us",
    "https://www.sozhaa.events/services/",
    "https://www.sozhaa.events/our-events/",
    "https://www.sozhaa.events/career/",
    "https://www.sozhaa.events/contact-us/"
]
...
def build_system_prompt(snippets):
    ...
    return (
        "You are Sozhaa Events AI Assistant. Use only the company's information provided below "
        "(sozhaa.events). Answer only about the company, services, pages and contact information. "
        "If asked outside scope, reply politely that you only provide Sozhaa Events info. "
...
def call_gemini(system_prompt, history, user_message):
    ...
    if "service" in user_message.lower() or "services" in user_message.lower():
        return (
            "Sozhaa Events provides a wide range of professional Events services 🚀.\n"
...
def build_html_email(user_details, service, transcript):
    header = f"""
    <h2>Sozhaa Events — Chat Transcript</h2>
...
async def chat_endpoint(payload: ChatPayload, background_tasks: BackgroundTasks):
    ...
    if "[User ended the chat]" in user_msg:
        assistant_text = "✅ Thank you for chatting with Sozhaa Events 🚀\nOur team will contact you soon."
        ...
        if user_phone:
            thank_msg = "✅ Thanks for contacting Sozhaa Events. Our team will contact you soon 🚀"
        ...
        if payload.user_details.get("email"):
            thank_you_html = html + "<br><br><p>🙏 Thank you for chatting with Sozhaa Events. Our team will connect with you soon.</p>"
            send_email_with_attachment(payload.user_details["email"], "Sozhaa Events — Chat Summary", thank_you_html, TRANSCRIPT_EXCEL)
    ...
    def save_and_notify():
        ...
        if payload.user_details.get("email"):
            send_email_with_attachment(payload.user_details["email"], "Sozhaa Events — Your Chat Transcript", html, TRANSCRIPT_EXCEL)
